...
import { assert, assertArrayContains, assertStrictEquals, assertThrows } from "https://deno.land/std@0.65.0/testing/asserts.ts"
...
Deno.test({
	...
	fn: function() {
		...
		for (let i = 0; i < 1000000; i++) {
			try {
				const environment = new Environment(people, 1, -1, 0, 2, 1, function(happinesses) {
					return happinesses.reduce(function(previous, current) {
						return previous + current;
					});
				});
				assertStrictEquals(environment.rooms.size, 4);
				assertArrayContains([...environment.rooms].flatMap(function(room) {
					return [...room.members];
				}), [...people]);
			} catch(error) {
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間12"),
				new Person("人間13")
			]), 1, -1, 0, 2, 1, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, RangeError, "人数（13）は3で割り切れる必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間11"),
				new Person("人間12"),
			]), 1, -1, 0, 6.5, 1, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, NotIntegerError, "attended roomsの数（6.5）は整数である必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間11"),
				new Person("人間12"),
			]), 1, -1, 0, 1, 1, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, RangeError, "attended roomsの数（1）は2～4である必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間11"),
				new Person("人間12"),
			]), 1, -1, 0, 5, 1, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, RangeError, "attended roomsの数（5）は2～4である必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間11"),
				new Person("人間12"),
			]), 1, -1, 0, 2, 1.5, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, NotIntegerError, "movable peopleの人数（1.5）は整数である必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間11"),
				new Person("人間12"),
			]), 1, -1, 0, 2, 0, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, RangeError, "movable peopleの人数（0）は１～３である必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		assertThrows(function() {
			new Environment(new Set([
				...
				new Person("人間11"),
				new Person("人間12"),
			]), 1, -1, 0, 2, 4, function(happinesses) {
				return happinesses.reduce(function(previous, current) {
					return previous + current;
				});
			});
		}, RangeError, "movable peopleの人数（4）は１～３である必要があります。");
	}
});
Deno.test({
	...
	fn: function() {
		...
		const environment = new Environment(new Set(people), 1, -1, 0, 2, 1, function(happinesses) {
			return happinesses.reduce(function(previous, current) {
				return previous + current;
			});
		});
		for (let i = 0; i < 1000000; i++) {
			...
			assert(state[0][0].likeCount >= 0 && state[0][0].likeCount <= 2, `${state[0][0].likeCount}は０～２のはずです。`);
			assert(state[0][0].dislikeCount >= 0 && state[0][0].dislikeCount <= 2, `${state[0][0].dislikeCount}は０～２のはずです。`);
			assert(state[0][1].likeCount >= 0 && state[0][1].likeCount <= 3, `${state[0][1].likeCount}は０～３のはずです。`);
			assert(state[0][1].dislikeCount >= 0 && state[0][1].dislikeCount <= 3, `${state[0][1].dislikeCount}は０～３のはずです。`);
			assert(state[1][0].likeCount >= 0 && state[1][0].likeCount <= 3, `${state[1][0].likeCount}は０～３のはずです。`);
			assert(state[1][0].dislikeCount >= 0 && state[1][0].dislikeCount <= 3, `${state[1][0].dislikeCount}は０～３のはずです。`);
			assert(state[1][1].likeCount >= 0 && state[1][1].likeCount <= 2, `${state[1][1].likeCount}は０～２のはずです。`);
			assert(state[1][1].dislikeCount >= 0 && state[1][1].dislikeCount <= 2, `${state[1][1].dislikeCount}は０～２のはずです。`);
...
Deno.test({
	...
	fn: function() {
		...
		const environment = new Environment(new Set(people), 1, -1, 0, 3, 2, function(happinesses) {
			return happinesses.reduce(function(previous, current) {
				return previous + current;
			});
		});
...
Deno.test({
	...
	fn: function() {
		...
		const environment = new Environment(new Set(people), 1, -1, 0, 2, 1, function(_happinesses) {
			return 9;
		});
