import { assertEquals, assertStrictEquals, assertThrows } from "https://deno.land/std@0.65.0/testing/asserts.ts";
...
Deno.test({
	...
	fn: function() {
		...
		assertThrows(function() {
			sato.likes(tanaka);
		}, ContradictoryImpressionsError, "佐藤は田中に対して矛盾した希望を持っています。");
		assertEquals(sato.likedPeople, new Set<Person>());
		assertEquals(sato.dislikedPeople, new Set([tanaka]));
	}
});
Deno.test({
	...
	fn: function() {
		...
		assertThrows(function() {
			tanaka.dislikes(suzuki);
		}, ContradictoryImpressionsError, "田中は鈴木に対して矛盾した希望を持っています。");
