...
export class Environment {
  ...
  constructor(
    ...
    if (!(m >= 1 && m <= 3)) {
      throw new RangeError(
        `movable peopleの人数（${m}）は１～３である必要があります。`,
      );
    }
    this.#m = m;
  ...
  receive(action: Action): number {
    if (
      !(action.memberIndex1 > 0 && action.memberIndex1 <= this.#m * this.#a)
    ) {
      throw new RangeError(
        `memberIndex1（${action.memberIndex1}）は${
          this.#m * this.#a
        }以下である必要があります。`,
      );
    }
    if (
      !(action.memberIndex2 > 0 && action.memberIndex2 <= this.#m * this.#a)
    ) {
      throw new RangeError(
        `memberIndex2（${action.memberIndex2}）は${
          this.#m * this.#a
        }以下である必要があります。`,
      );
    }
    ...
    const room1 = this.#attendedRooms.find((attendedRoom) => {
      return attendedRoom.members.has(
        this.#movablePeople[action.memberIndex1 - 1],
      );
    })!;
    const room2 = this.#attendedRooms.find((attendedRoom) => {
      return attendedRoom.members.has(
        this.#movablePeople[action.memberIndex2 - 1],
      );
    })!;
